...
class grade_report_grader extends grade_report {
    ...
    public function load_users(bool $allusers = false) {
        ...
        if (empty($this->users)) {
            ...
            // Augment users with course last access.
            list($usql2, $uparams2) = $DB->get_in_or_equal(array_keys($this->users), SQL_PARAMS_NAMED, 'ulast');
            $paramsla = array_merge(['courseid' => $this->courseid], $uparams2);
            $sql = "SELECT userid, MAX(timeaccess) AS timeaccess
                      FROM {user_lastaccess}
                     WHERE courseid = :courseid AND userid $usql2
                  GROUP BY userid";
            $lastaccess = $DB->get_records_sql($sql, $paramsla);
            foreach ($this->users as $user) {
                $this->users[$user->id]->courselastaccess = isset($lastaccess[$user->id]) ? (int)$lastaccess[$user->id]->timeaccess : null;
            }
            ...
            $sql = "SELECT id, lastlogin
                      FROM {user}
                     WHERE id $usql2";
            $userlogins = $DB->get_records_sql($sql, $uparams2);
            foreach ($this->users as $user) {
                $this->users[$user->id]->lastlogin = isset($userlogins[$user->id]) ? (int)$userlogins[$user->id]->lastlogin : null;
            }

            // Activities completed count per user (course modules with completionstate = 1).
            $sql = "SELECT cmc.userid, COUNT(1) AS cnt
                      FROM {course_modules_completion} cmc
                      JOIN {course_modules} cm ON cm.id = cmc.coursemoduleid
                     WHERE cm.course = :courseid AND cmc.userid $usql2 AND cmc.completionstate = 1
                  GROUP BY cmc.userid";
            $paramscomp = array_merge(['courseid' => $this->courseid], $uparams2);
            $completed = $DB->get_records_sql($sql, $paramscomp);
            foreach ($this->users as $user) {
                $this->users[$user->id]->activitiescompleted = isset($completed[$user->id]) ? (int)$completed[$user->id]->cnt : 0;
            }

            // Assignment submissions count per user in this course.
            $sql = "SELECT s.userid, COUNT(1) AS cnt
                      FROM {assign_submission} s
                      JOIN {assign} a ON a.id = s.assignment
                     WHERE a.course = :courseid AND s.userid $usql2 AND s.status = 'submitted'
                  GROUP BY s.userid";
            $paramssub = array_merge(['courseid' => $this->courseid], $uparams2);
            $subs = $DB->get_records_sql($sql, $paramssub);
            foreach ($this->users as $user) {
                $this->users[$user->id]->assignmentsubmissions = isset($subs[$user->id]) ? (int)$subs[$user->id]->cnt : 0;
            }
        }
        return $this->users;
    }
    ...
    public function get_left_rows($displayaverages) {
        ...
        // Add custom columns: last login, last course access, activities completed, assignment submissions.
        $customcols = 4;
        $colspan = 1 + count($extrafields) + $customcols;
        ...
        // Custom headers.
        $lastloginhdr = new html_table_cell();
        $lastloginhdr->attributes['class'] = 'userfield userlastlogin';
        $lastloginhdr->scope = 'col';
        $lastloginhdr->header = true;
        $lastloginhdr->text = get_string('lastlogin', 'gradereport_grader');
        $headerrow->cells[] = $lastloginhdr;

        $lastcourseaccesshdr = new html_table_cell();
        $lastcourseaccesshdr->attributes['class'] = 'userfield userlastcourseaccess';
        $lastcourseaccesshdr->scope = 'col';
        $lastcourseaccesshdr->header = true;
        $lastcourseaccesshdr->text = get_string('lastcourseaccess', 'gradereport_grader');
        $headerrow->cells[] = $lastcourseaccesshdr;

        $activitieshdr = new html_table_cell();
        $activitieshdr->attributes['class'] = 'userfield useractivitiescompleted';
        $activitieshdr->scope = 'col';
        $activitieshdr->header = true;
        $activitieshdr->text = get_string('activitiescompleted', 'gradereport_grader');
        $headerrow->cells[] = $activitieshdr;

        $assignmentshdr = new html_table_cell();
        $assignmentshdr->attributes['class'] = 'userfield userassignmentsubmissions';
        $assignmentshdr->scope = 'col';
        $assignmentshdr->header = true;
        $assignmentshdr->text = get_string('assignmentsubmissions', 'gradereport_grader');
        $headerrow->cells[] = $assignmentshdr;
        ...
        foreach ($this->users as $userid => $user) {
            ...
            // Last login.
            $lastlogintext = empty($user->lastlogin) ? '-' : userdate($user->lastlogin, get_string('strftimedatetimeshort'));
            $cell = new html_table_cell();
            $cell->attributes['class'] = 'userfield userlastlogin';
            $cell->header = false;
            $cell->text = html_writer::tag('div', $lastlogintext, ['data-collapse' => 'content']);
            $userrow->cells[] = $cell;

            // Last course access.
            $lastcourseaccess = $user->courselastaccess ?? null;
            $lastcourseaccesstext = empty($lastcourseaccess) ? '-' : userdate($lastcourseaccess, get_string('strftimedatetimeshort'));
            $cell = new html_table_cell();
            $cell->attributes['class'] = 'userfield userlastcourseaccess';
            $cell->header = false;
            $cell->text = html_writer::tag('div', $lastcourseaccesstext, ['data-collapse' => 'content']);
            $userrow->cells[] = $cell;

            // Activities completed count.
            $activitiescompleted = (int)($user->activitiescompleted ?? 0);
            $cell = new html_table_cell();
            $cell->attributes['class'] = 'userfield useractivitiescompleted';
            $cell->header = false;
            $cell->text = html_writer::tag('div', (string)$activitiescompleted, ['data-collapse' => 'content']);
            $userrow->cells[] = $cell;

            // Assignment submissions count.
            $assignmentsubmissions = (int)($user->assignmentsubmissions ?? 0);
            $cell = new html_table_cell();
            $cell->attributes['class'] = 'userfield userassignmentsubmissions';
            $cell->header = false;
            $cell->text = html_writer::tag('div', (string)$assignmentsubmissions, ['data-collapse' => 'content']);
            $userrow->cells[] = $cell;
...
/**
 * Get comprehensive analytics data for users in a course
 * Integrated directly into grader report
 *
 * @param int $courseid Course ID
 * @param array $userids Array of user IDs
 * @return array Analytics data
 */
function gradereport_grader_get_analytics_data($courseid, $userids) {
    global $DB;
 ...
 */
function gradereport_grader_get_assignment_analytics($courseid, $userids, &$analytics) {
    global $DB;
 ...
 */
function gradereport_grader_get_interactive_content_analytics($courseid, $userids, &$analytics) {
    global $DB;
 ...
 */
function gradereport_grader_get_forum_analytics($courseid, $userids, &$analytics) {
    global $DB;
 ...
 * Get attendance data
 */
function gradereport_grader_get_attendance_analytics($courseid, $userids, &$analytics) {
    global $DB;
 ...
 */
function gradereport_grader_get_competency_analytics($courseid, $userids, &$analytics) {
    global $DB;
 ...
 */
function gradereport_grader_get_badge_analytics($courseid, $userids, &$analytics) {
    global $DB;
 ...
 */
function gradereport_grader_get_behavioral_analytics($courseid, $userids, &$analytics) {
    global $DB;
 ...
 */
function gradereport_grader_get_ta_evaluation_analytics($courseid, $userids, &$analytics) {
    global $DB;
