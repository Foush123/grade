...
/**
 * Settings definition (existing code)
 */
...
/**
 * Profile report callback (existing)
 */
...
/**
 * Add nodes to myprofile page (existing)
 */
...
/**
 * Returns link to user report for the current element (existing)
 */
...
/***********************************************
 * NEW HELPER FUNCTIONS FOR EXTRA FEATURES
 ***********************************************/

/**
 * Get last login timestamp for a user.
 */
function gradereport_user_get_last_login(int $userid) {
    global $DB;

    $lastlogin = $DB->get_field('user', 'lastlogin', ['id' => $userid]);
    if ($lastlogin) {
        return userdate($lastlogin);
    }
    return get_string('never');
}

/**
 * Get last access for each activity for a user in a course.
 */
function gradereport_user_get_activity_access(int $userid, int $courseid) {
    global $DB;
    ...
    foreach ($modinfo->cms as $cm) {
        $lastaccess = $DB->get_field('logstore_standard_log', 'MAX(timecreated)', [
            'userid' => $userid,
            'contextid' => $cm->contextid,
            'action' => 'viewed'
        ]);
        $access[$cm->id] = $lastaccess ? userdate($lastaccess) : get_string('never');
    }
...
/**
 * Get time spent per activity for a user.
 */
function gradereport_user_get_time_spent(int $userid, int $courseid) {
    global $DB;

    $sql = "SELECT contextinstanceid AS cmid, SUM(LEAD(timecreated) OVER 
            (PARTITION BY contextinstanceid ORDER BY timecreated) - timecreated) AS timespent
    ...
    foreach ($records as $r) {
        $result[$r->cmid] = $r->timespent ?? 0;
    }
    return $result;
}

/**
 * Get list of assignments submitted by the user in a course.
 */
function gradereport_user_get_assignments(int $userid, int $courseid) {
    global $DB;

    $assignments = $DB->get_records_sql("
        SELECT a.id AS assignid, s.status
        FROM {assign} a
        LEFT JOIN {assign_submission} s
        ON a.id = s.assignment AND s.userid = :userid
